          ...
          <div>
            ...
            >
              ...
              <AccordionItem className={styles.accordionItem} eventKey="4">
                ...
                <AccordionBody className={styles.accordionBody}>
                  {/** @ts-ignore */}
                  <Trans t={t} components={{ Link: <Link ahref=""></Link> }}>
                  ...
                  <Col className={styles.linkColumn} key={i} sm={6} md={4}>
                    ...
                    <p>
                      <a
                        ...
                        href={t(`support.tipsExperts.tips.${i}.link.url`)}
                      >
                        {t(`support.tipsExperts.tips.${i}.link.title`)}
                      </a>
                    </p>
                  </Col>
                ))}
              ...
              <Row className={styles.linkWrapper}>
                <Col md={6} className={styles.linkColumn}>
                  {[1, 2, 3, 4, 5].map((_, index) => {
                    return (
                      <div key={`${index}-1`} className={styles.linkWrapper}>
                        <h5>{t(`support.links.${index}.title`)}</h5>
                        <Link
                          className={styles.hrefWrapper}
                          href={t(`support.links.${index}.url`)}
                        >
                          {t(`support.links.${index}.title`)}
                        </Link>
                      </div>
                    )
                  })}
                ...
                <Col md={6} className={styles.linkColumn}>
                  {[1, 2, 3, 4, 5].map((_, index) => {
                    return (
                      <div key={`${index}-2`} className={styles.linkWrapper}>
                        <h5>{t(`support.links.${index}.title`)}</h5>
                        <Link
                          className={styles.hrefWrapper}
                          href={t(`support.links.${index}.url`)}
